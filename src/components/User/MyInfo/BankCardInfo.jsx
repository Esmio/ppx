...
import { SLIcon, MDIcon, OrangeButton, RedButton } from '../../General';
...
class BankCardInfo extends Component {
  ...
  renderFormLabel() {
    const { bankCardExist, bankName, cardIsDefault } = this.props;
    if (bankCardExist) {
    ...
    return <h4 className={css.profile_formLabel}>银行卡信息</h4>;
  }
  renderBtnRow() {
      ...
      <div>
        {
          ...
          <div className={css.profile_contentBody}>
            <h4 className={css.profile_formLabel}>完善提款信息</h4>
...
const mapStatesToProps = ({ userModel, formModel }) => {
  const { bankAccounts, userData, selectedBankCardId } = userModel;
  return { selectedBankCardId, bankAccounts, userData, ...formModel };
};
