...
class OrderExpensesRecord extends Component {
  ...
  componentWillMount() {
    this.dispatch({ type: 'orderModel/updateState', payload: { state: 'WIN' } });
  ...
  componentWillUnmount() {
    ...
    this.dispatch({
      type: 'dataTableModel/initializeState', payload: ['state', 'orderHistory']
    });
  }
  ...
  sliceList({ orderHistory, pageSize, currentPage }) {
    ...
    if (orderHistory) {
      if (sorting === 'up') {
        currentList = _.sortBy([...currentList], ['winAmount']);
      } else if (sorting === 'down') {
        currentList = _.reverse(_.sortBy([...currentList], ['winAmount']));
      }
      if (searchText) {
  ...
  renderTableBody() {
    ...
    if (currentList.length) {
      return _.map(currentList, (listItem) => {
        const {
          transactionTimeuuid, winAmount, bettingTime,
        } = listItem;
        const stringArry = getConcatArray(transactionTimeuuid);
        return (!winAmount ? null :
    ...
    return (
      <div>
        <div className={css.profile_contentBody}>
          <h4 className={css.profile_formLabel}>奖金派送
...
const mapStatesToProps = ({ dataTableModel, orderModel }) => {
  return { ...dataTableModel, ...orderModel };
};
