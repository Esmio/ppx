...
class DigitInput extends Component {
  ...
  renderIndicate() {
    const {
      readOnly, value, max, dataColor, dataMessage, dataIcon, type
    } = this.props;
    if (readOnly || !value) {
      return null;
    }
      ...
      <div>
        <p
          className={dataMessage ?
            css.profile_inputMsg__show : css.profile_inputMsg
          }
        ...
        { max ?
          <span className={css.profile_inputLength} >
            { type !== 'number' ? <span>{value.length || 0}</span> : null } / { max }
          </span> : null
        }
      </div>
    );
  ...
  render() {
    const {
      readOnly, value, label, type, dataColor, name, min, max, pattern,
