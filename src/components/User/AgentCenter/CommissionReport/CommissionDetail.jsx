  ...
  renderTableBody() {
    ...
    if (commissionDetailCount) {
      return _.map(commissionDetail, (listItem) => {
        const {
          taskIdentifier, agentChildUsername, sumBetsTaskPeriod, agentCommission
        } = listItem;
        return (
          <tr key={`${agentChildUsername}_${sumBetsTaskPeriod}`}>
            <td>{ taskIdentifier }</td>
            <td>{ agentChildUsername }</td>
      ...
      <div className={css.profile_contentBody}>
        <h4 className={css.profile_formLabel}>
          ...
          <span disabled className={css.profile_breadcrumItem}>
            <MDIcon iconName="chevron-right" /><i>第{ taskIdentifier }期佣金详情</i>
          </span>
        ...
        <table className={css.profile_table}>
          <thead>
            <tr>
              <td width="30%">期号</td> 
              <td width="30%">用户名</td>
              <td width="20%" data-align="right">投注金额</td>
              <td width="20%" data-align="right">佣金</td>
            </tr>
          </thead>
          <tbody>
