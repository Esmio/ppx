...
class CommissionList extends Component {
      ...
      <ul className={css.profile_dropdownMenu}>
        {
          _.map(commissionStatusRefs, (typeValue, typeKey) => {
            ...
            return (
              <button {...btnProps}>{ typeValue }</button>
            );
          })
        }
      </ul>
  ...
  renderDateThead() {
    const {
      dayCounts, timeframeRefs, manualTimeframe, startTime, endTime
    } = this.props;
    const ranges = {};
  ...
  renderTableBody() {
    ...
    if (myCommissionHistory.length) {
      return _.map(myCommissionHistory, (listItem) => {
        const { id, createdTime, issueNo, betsSum, commission, userCount, status } = listItem;
        return (
          <tr key={id}>
  ...
  renderTotalCount() {
    ...
    const shouldShowTotal = (totalBetsSum || totalCommission) && myCommissionHistory.length;
    if (shouldShowTotal) {
      return (
        <tr className={css.profile_tableTotalRow}>
          <td colSpan="4" data-align="right">总结</td>
            ...
            <tr>
              ...
              <td>
                <Dropdown overlay={this.renderStatusDropdown()}>
                  <button>
                    {status === 'ALL' ? '交易状态' : commissionStatusRefs[status]}
                    <MDIcon iconName="menu-down" className={css.profile_tableMenuDownIcon} />
                  </button>
                </Dropdown>
              </td>
