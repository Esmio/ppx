...
function ifSymbolicAware({ symbolic }) {
...
export const getOpenOptionsString = ((currentOpenOptions) => {
  let optionsString = '';
  _.forEachRight(type.OPEN_OPTION_STRINGS, (string, index) => {
    if (currentOpenOptions.indexOf(index) >= 0) {
      optionsString = `${optionsString} ${string}`;
    }
  });
...
export const getDisplayString = (({ currentPicks, gameSetCombination }) => {
  const {
    joinPickWith,
    joinSectionWith
  } = gameSetCombination;
  ...
  let displayString = '';
  let picksArray = [];
  const sectionJoiner = joinSectionWith || '|';
  const currentPickKeys = _.keys(currentPicks);
  _.forEach(currentPicks, (picks, picksKey) => {
    let pickString = '';
    const pickJoiner = (joinPickWith && joinPickWith[_.indexOf(currentPickKeys, picksKey)]) || ' ';
    _.forEach(picks, pick => {
      pickString = `${pickString}${pick}${pickJoiner}`;
    });
    picksArray.push(pickString);
  });
  picksArray = _.map(picksArray, _.trim);
  displayString = _.join(picksArray, sectionJoiner);
...
export const generateBetQuantity = (({
  selectedGameSetting, picksOnGame, openOptionsOnGame
}) => {
  function getEachPicksLength() {
    ...
    _.forEach(this.gameSections, (section) => {
      let length = 0;
      _.forEach(this.picksOnGame[section], string => {
        if (string !== '') {
          length++;
        }
      });
  ...
  function getValueRef(gameId) {
    return type.VALUE_REF[gameId];
  }

  function init() {
    this.gameSetCombination = selectedGameSetting.gameSetCombination;
    this.gameSections = this.gameSetCombination.sections;
    this.formula = this.gameSetCombination.formula;
    this.uniqueInt = this.gameSetCombination.uniqueInt;
    this.isUnique = this.gameSetCombination.isUnique;
    this.minimumRowPick = this.gameSetCombination.minimumRowPick;
    this.pickRange = this.gameSetCombination.pickRange;
    this.openOptions = this.gameSetCombination.openOptions;
    this.valueRef = getValueRef(selectedGameSetting.gameId);
  ...
  function validateSet() {
    ...
    _.forEach(this.picksLength, (length, index) => {
      ...
      const conditionPassed = length >= min && length <= max;
        // console.debug(range, min, max, length, conditionPassed);
      if (conditionPassed) {
        condition++;
      }
    });
...
export const getBetQuantity = (({
  ...
  const betQuantity = generateBetQuantity({
    selectedGameSetting, picksOnGame, openOptionsOnGame
  })
  .validate()
  .calculate()
...
export const getCalculateDetails = (({
  ...
  const picksOnGame = existingPicks[selectedGameName] || {};
